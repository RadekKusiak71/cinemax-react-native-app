...
const SeatSelectionView = () => {
    ...
    const handleBookSeats = () => {
        ...
        createReservation({ showtimeId: numericShowtimeId, seatIds: selectedSeats }, {
            onSuccess: (data) => {
                router.push({
                    pathname: `/(movie)/(booking)/[showtime_id]/confirmation`,
                    params: { reservation_id: data.id.toString(), showtime_id: showtime_id as string }
                });
            },
            onError: () => {
                Alert.alert("Failed to create reservation. Please try again.");
            }
        });
    };
